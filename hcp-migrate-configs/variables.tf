...
variable "migration_directories" {
  default = {
    CostInsight = {
      ...
      current_backend    = "s3"
      git_commit_message = "[skip ci] Migrating local Workspaces from CostInsight to TFC"
      project_name       = "cost_insight"
      tags               = ["cost_insight"]
      working_directory  = "/Users/vigneshjayabalan/hashicorp/src/github/vignesh.jayabalan/Miracle-Holdings-Inc/CostInsight"
      workspace_map = {
        default    = "cost_insight_default"
        dev        = "cost_insight_dev"
        prod       = "cost_insight_prod"
        production = "cost_insight_production"
        stage      = "cost_insight_stage"
      }
    }
    CreditBoost = {
      backend_file_name  = "main.tf"
